...
if HTML_ONLY:
    ...
    # BANKS_JSON from all non-agg parquets — use DuckDB (hist has 64M+ rows; pandas OOMs)
    import duckdb as _ddb2
    _roster_p=[p for p in _all_p if p not in AGG_PARTS]
    _rp_list="["+",".join(f"'{os.path.join(SITE,p)}'" for p in _roster_p)+"]"
    ros=_ddb2.sql(f"SELECT id_rssd, LAST(institution_name ORDER BY quarter_end) AS institution_name FROM read_parquet({_rp_list}) GROUP BY id_rssd ORDER BY id_rssd").df()
    banks=[[int(r.id_rssd), str(r.institution_name) if r.institution_name else str(r.id_rssd)] for r in ros.itertuples()]
    ...
    if _ap:
        _ap_list="["+",".join(f"'{os.path.join(SITE,p)}'" for p in _ap)+"]"
        _max_q=_ddb2.sql(f"SELECT max(quarter_end) FROM read_parquet({_ap_list})").fetchone()[0]
        active_rssds=set(r[0] for r in _ddb2.sql(f"SELECT DISTINCT id_rssd FROM read_parquet({_ap_list}) WHERE quarter_end='{_max_q}'").fetchall())
    ...
    _all_list="["+",".join(f"'{os.path.join(SITE,p)}'" for p in _all_p)+"]" if _all_p else None
    _mdrms_in_site=set(r[0] for r in _ddb2.sql(f"SELECT DISTINCT mdrm FROM read_parquet({_all_list})").fetchall()) if _all_list else set()
    NODATA_CODES=sorted(c for c in _dc2 if c and c not in _mdrms_in_site)
else:
  ...
  # Pre-build DISPLAY_CODES so we can pre-filter during parquet load.
  # The panel has 141M+ rows after the 1986 extension; pd.read_parquet on the full file OOMs.
  # DuckDB reads with a WHERE clause, returning only the ~10M rows the dashboard needs.
  DISPLAY_CODES=set()
  ...
  _codes_csv=",".join(f"'{c}'" for c in sorted(DISPLAY_CODES)) if DISPLAY_CODES else "'__none__'"
  df=_ddb.sql(f"SELECT * FROM read_parquet('{SRC}') WHERE mdrm IN ({_codes_csv})").df()
  print(f"DuckDB pre-filtered load: {len(df):,} rows ({len(DISPLAY_CODES)} display codes)")
  ...
  <div><label>Measures (click items in the left rail; ✕ to remove)</label> <button id="clrmeas" class="sec" style="padding:1px 6px;font-size:12px;opacity:0.6" title="Remove all measures">✕ Clear</button> <button id="calcbtn" class="sec" style="padding:1px 6px;font-size:12px" title="Define a custom calculated series combining existing line items">Σ Calc</button>
   ...
   <div id="calcdiv" style="display:none;margin:4px 0 8px;padding:8px 10px;border:1px solid var(--border,#ccc);border-radius:6px;font-size:13px;background:var(--bg2,#f7f9fb)">
    ...
    <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;align-items:center">
     <button id="calcSave" class="sec" style="font-size:13px;padding:2px 8px" title="Download all custom formulas as fry9c_formulas.json">⬇ Save to file</button>
     <button id="calcLoad" class="sec" style="font-size:13px;padding:2px 8px" title="Load formulas from a saved fry9c_formulas.json file">⬆ Load from file</button>
...
// HC-N item 9 leaf codes (items 1-8 only, 22 per column) — used as fallback for the
// 1990-Q3–2017-Q4 reporting gap where BHCK1406/1407/1403 totals were not collected.
const _HCN9_A=['BHCKF172','BHCKF173','BHCK3493','BHCK5398','BHCKC236','BHCKC238','BHCK3499','BHCKF178','BHCKF179','BHCKB572','BHCK5377','BHCK5380','BHCK1594','BHCK1606','BHCKB575','BHCKK213','BHCKK216','BHCK5389','BHCK5459','BHCKF166','BHCKF169','BHCKKX63'];
const _HCN9_B=['BHCKF174','BHCKF175','BHCK3494','BHCK5399','BHCKC237','BHCKC239','BHCK3500','BHCKF180','BHCKF181','BHCKB573','BHCK5378','BHCK5381','BHCK1597','BHCK1607','BHCKB576','BHCKK214','BHCKK217','BHCK5390','BHCK5460','BHCKF167','BHCKF170','BHCKKX64'];
const _HCN9_C=['BHCKF176','BHCKF177','BHCK3495','BHCK5400','BHCKC229','BHCKC230','BHCK3501','BHCKF182','BHCKF183','BHCKB574','BHCK5379','BHCK5382','BHCK1583','BHCK1608','BHCKB577','BHCKK215','BHCKK218','BHCK5391','BHCK5461','BHCKF168','BHCKF171','BHCKKX65'];
const DERIV={
 ...
 'D_NPL':{type:'hybrid_ratio',lbl:'Credit ▸ NPL % (Past Due + Non Accrual / loans)',parts:[{reported:'BHCK1406',components:_HCN9_A},{reported:'BHCK1407',components:_HCN9_B},{reported:'BHCK1403',components:_HCN9_C}],den:['BHCK2122']},
 'D_NONCUR':{type:'hybrid_ratio',lbl:'Credit ▸ Noncurrent ratio % (90+PD+nonaccrual / loans)',parts:[{reported:'BHCK1407',components:_HCN9_B},{reported:'BHCK1403',components:_HCN9_C}],den:['BHCK2122']},
...
// present in this engine's own DERIV catalog (or, for raw BHCK codes, resolves via seriesFor()'s
// raw-mdrm fallback — see make_site_fry9c.py review notes). Y-9C has NO liquidity/funding-ratio DERIV
// entries the way Call/002 do (Schedule HC doesn't break out securities/cash/borrowings at the DERIV
// layer) — per UI_EFFICIENCY_PLAN.md §1 gap note, do NOT port Call's D_LIQ_ASSETS/D_BROKERED/etc. here.
// xform keys map 1:1 to the Transform cluster checkbox ids; range is '1Y'|'5Y'|'10Y'|'All'.
const VIEWS=[
 {id:'liquidity',name:'Liquidity snapshot',icon:'💧',desc:'Loans/deposits and deposits/assets — Y-9C has no liquid-assets ratio at the DERIV layer yet.',measures:['D_LOANSDEP','D_DEPASSETS'],xform:{},range:'5Y'},
 {id:'credit',name:'Credit quality',icon:'⚠️',desc:'NPL %, noncurrent ratio, reserve coverage, net charge-off rate.',measures:['D_NPL','D_NONCUR','D_RESCOV','D_NCO_LOANS_Y9C'],xform:{},range:'5Y'},
 {id:'capital',name:'Capital & earnings',icon:'🏦',desc:'Equity/assets, ROA, ROE, NIM, efficiency ratio.',measures:['D_EQASSETS','D_ROA','D_ROE','D_NIM','D_EFF'],xform:{},range:'5Y'},
 {id:'funding',name:'Funding mix',icon:'💰',desc:'Deposits/assets and total deposits — Y-9C has no borrowings/brokered-deposit ratios at the DERIV layer.',measures:['D_DEPASSETS','S_DEP'],xform:{normbyassets:true,normden:'BHCK2170'},range:'5Y'},
 {id:'loanbook',name:'Loan quality by book',icon:'📊',desc:'NPL % broken out by loan book: C&I, credit card, auto, consumer, ag, residential, construction, multifamily, CRE.',measures:['D_NPL_CI','D_NPL_CC','D_NPL_AUTO','D_NPL_CONS','D_NPL_AG','D_NPL_RES','D_NPL_CONSTR','D_NPL_MULTI','D_NPL_CRE'],xform:{},range:'5Y'},
 {id:'balsheet',name:'Balance sheet levels',icon:'📈',desc:'Assets, loans, deposits, equity indexed to 100 — long-run growth trend.',measures:['BHCK2170','BHCK2122','S_DEP','BHCK3210'],xform:{idx:true},range:'10Y'},
 {id:'reserve',name:'Reserve build',icon:'🛡️',desc:'Reserves/loans and reserve coverage of noncurrent loans over a long window.',measures:['D_RESLOANS','D_RESCOV'],xform:{},range:'10Y'},
 {id:'peercompare',name:'Peer compare — size bucket',icon:'⚖️',desc:'NPL % as a starting lens for comparing against ALL or a peer group you pick.',measures:['D_NPL'],xform:{},range:'5Y'},
];
...
const DKIND=m=>(DERIV[m]||DYN[m]||USERCALC[m])?.type||null;const isPct=m=>DKIND(m)==='ratio'||DKIND(m)==='hybrid_ratio'||(USERCALC[m]?.type==='expr'&&!!USERCALC[m]?.pct);
...
async function seriesFor(id,m){const cond=scopeCond(id);if(cond==null)return [];
 ...
 if(d&&(d.type==='hybrid_ratio'||d.type==='hybrid_sum')){
   const allCodes=[...new Set([...d.parts.flatMap(p=>[p.reported,...p.components]),...(d.den||[]).flatMap(term2codes)])];
   const r=(await conn.query(`SELECT id_rssd,quarter_end,mdrm,value FROM t WHERE ${cond} AND mdrm IN (${sqlList(allCodes)})`)).toArray();
...
async function init(){try{
 ...
 document.getElementById('calcadd').onclick=()=>{
   ...
   try{localStorage.setItem('fry9c_formulas',getFormulasJson());}catch(_){}
   _seriesCache.clear();
...
function saveFormulas(){const s=getFormulasJson();const b=new Blob([s],{type:'application/json'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download='fry9c_formulas.json';a.click();URL.revokeObjectURL(u);showToast('Formulas downloaded as fry9c_formulas.json.','ok');try{localStorage.setItem('fry9c_formulas',s);}catch(_){}}
function loadFormulas(){document.getElementById('calcImportFile').click();}
function autoLoadFormulas(){try{const s=localStorage.getItem('fry9c_formulas');if(s)applyFormulas(JSON.parse(s));}catch{}}
 document.getElementById('calcSave').onclick=saveFormulas;
 document.getElementById('calcLoad').onclick=loadFormulas;
 document.getElementById('calcImportFile').onchange=e=>{const f=e.target.files?.[0];if(!f)return;const rd=new FileReader();rd.onload=ev=>{try{const obj=JSON.parse(ev.target.result);const n=applyFormulas(obj);showToast('Loaded '+n+' formula'+(n===1?'':'s')+'.','ok');try{localStorage.setItem('fry9c_formulas',ev.target.result);}catch(_){}}catch(ex){showToast('Load failed: invalid JSON file.','err');}};rd.readAsText(f);e.target.value='';};
...
async function applyView(vid){const v=VIEWS.find(x=>x.id===vid);if(!v)return;
 ...
 for(const code of v.measures){
   ...
   const pct=isPct(code)||PCTC.has(code);
...
function pane(series,pct,unit,win,stacked){const W=1080,pad=64,n=win.length;const _showL=window._inlineLbls!==false;const _gut=16;const _vbw=W+_gut;const _pinned=window._chartW>40&&window._chartH>40;let H=_pinned?Math.round(_vbw*window._chartH/window._chartW):420;H=Math.max(260,Math.min(2800,H));const _svgsty=(_pinned?'width:100%;height:100%':'width:100%;height:auto')+';display:block';const xi=Object.fromEntries(win.map((q,i)=>[q,i]));
 ...
 if(window._hybridOnChart){const gi0=win.findIndex(q=>q>='1990-09-30');const gi1=win.reduceRight((a,q,i)=>a<0&&q<='2017-12-31'?i:a,-1);if(gi0>=0&&gi1>=0&&gi0<=gi1){const gx=X(gi0),gx2=X(gi1);const gapC=DK()?'rgba(100,116,139,0.10)':'rgba(100,116,139,0.07)';tk+=`<rect x="${gx.toFixed(1)}" y="${pad}" width="${Math.max(2,gx2-gx).toFixed(1)}" height="${H-2*pad}" fill="${gapC}" stroke-dasharray="2 2" stroke="${DK()?'#475569':'#94a3b8'}" stroke-width="0.5"></rect><text x="${((gx+gx2)/2).toFixed(1)}" y="${(pad-12).toFixed(1)}" font-size="8" fill="${DK()?'#94a3b8':'#64748b'}" text-anchor="middle" opacity=".75">reconstructed</text>`;}}
...
async function renderLeague(){
 ...
 else{[vals,avals]=await Promise.all([perFilerValues(meas.code,allFetchQs),bkt?perFilerValues('BHCK2170',[q]):Promise.resolve(null)]);_lgCache={key:_lgKey,vals,avals};}
...
async function buildReport(rssd,latestQ,qtrs){
 ...
 const npl30=get('BHCK1406'),npl90=get('BHCK1407'),nona=get('BHCK1403');
