...
async function saveFormulas(){try{const r=await fetch('/api/save-formulas',{method:'POST',headers:{'Content-Type':'application/json'},body:getFormulasJson()});if(!r.ok)throw new Error('HTTP '+r.status);showToast('Formulas saved.','ok');}catch(e){try{localStorage.setItem('fry9c_formulas',getFormulasJson());showToast('Formulas saved (local).','ok');}catch(le){const el=document.getElementById('calcstatus');if(el)el.textContent='Save failed: '+le.message;}}}
async function loadFormulas(){try{const r=await fetch('/api/formulas');if(!r.ok)throw new Error('HTTP '+r.status);const obj=await r.json();const n=applyFormulas(obj);showToast('Loaded '+n+' formula'+(n===1?'':'s')+'.','ok');}catch(e){try{const s=localStorage.getItem('fry9c_formulas');if(!s)throw new Error('no local data');const n=applyFormulas(JSON.parse(s));showToast('Loaded '+n+' formula'+(n===1?'':'s')+' (local).','ok');}catch(le){const el=document.getElementById('calcstatus');if(el)el.textContent='Load failed: no server or local data.';}}}
